...
impl SendResponse for ClassInfo {
    async fn send_response(ctx: &Context, msg: &Message, _type: Vec<&str>) -> CommandResult {
        if _type[0] != "all" {
            ...
            let mut title: String = a.reference.name.clone();
            if a.hit_die != -1 {
                title += &*format!(" ({} hit(s) to death)", a.hit_die);
            }
            if !a.class_levels.is_empty() {
                title += &*format!(" (Level resource: {}{})", API_SERVER, a.class_levels);
            }
            ...
            for option in a.starting_equipment_options {
                starting_equipment_options += option.display(0).await.as_str();
            }
            ...
            if spellcasting.is_empty() {
                spellcasting = nil.clone()
            }
            ...
            while let Some(user_interactions) = interaction.next().await {
                ...
                if option == "Spellcasting" {
                    ...
                    for info in &a.spellcasting.info {
                        sp_menu_option.push(CreateSelectMenuOption::new(
                            &info.feature_type,
                            &info.feature_type,
                        ));
                    }
                            ...
                            .create_response(
                                ...
                                CreateInteractionResponse::UpdateMessage(
                                    CreateInteractionResponseMessage::default()
                                        ...
                                        .ephemeral(false)
                                        .content(format!(
                                            "Spellcasting: {} for class {}",
                                            sp_option, a.reference.name
                                        ))
                                        .embed(
                                            CreateEmbed::new()
                                                .title(format!(
                                ...
                                CreateInteractionResponseMessage::default()
                                    ...
                                    .embed(
                                        CreateEmbed::new()
                                            .title(format!(
                                                "{} for class {}",
                                                option, a.reference.name
                                            ))
                                            .field("", option_value, false),
                                    ),
                            ),
                        )
                        .await
            ...
            let mut embed = CreateEmbed::new().title("**All available Classes**");
