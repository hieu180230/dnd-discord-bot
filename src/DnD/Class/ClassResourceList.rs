        ...
        match json.get("results") {
            Some(T) => {
                for reference in T.as_array().unwrap() {
                    self.results.push(APIReference {
...
pub async fn send_class_resource_response(
    ctx: &Context,
    msg: &Message,
    _type: Vec<String>,
) -> CommandResult {
    ...
    let res = client
        .get(format!("{}{}", API_SERVER, API_Endpoint))
        .send()
        .await
        ...
        match option.as_str() {
            "first" => {
                ...
                for i in 0..display_data[count].len() {
                    embed = embed.field(
                        &*display_data[count][i].name,
                        &*display_data[count][i].index,
                        true,
                    );
                }
            }
